...
- **画面中部偏下会打一枚 `ijinhua.com` 水印**：水平居中，文字中心落在**距底边 1/4 图高**
  处（= 图高的 3/4，改位置就改 `prep_photos.py` 的 `WM_Y_RATIO`）。缩完尺寸之后打，
  字号跟最终宽度走；不想要加 `--no-watermark`。理由很实在：图一旦离开这个站就没头没尾了，
  被人存下来再转发，谁也认不出是哪来的。（原先贴在右下角，2026-09-24 挪到中下。）
