...
REGIONS = ["婺城区", "金东区", "兰溪市", "义乌市", "东阳市",
           "永康市", "武义县", "浦江县", "磐安县"]
# 2026-09-24 起只有一种类目。原先有瀑布/事件点/荷花/一堆秋色树种，图例上一排颜色太花，
# 站主让全并成「打卡点」—— 那张排色相、找空档的功课暂时用不上了，方法留在 app.js 的注释里。
# 想再分类目就往这里加；app.js 的 KIND_COLOR 是同一张表，改一边必须改另一边，
# 对不上的话点会拿到兜底色（现在就一种色，怎么都不至于没颜色）。
KINDS = ["打卡点"]
SEGS = {"上": 0, "中": 1, "下": 2}
...
def validate(data, photos_dir):
    ...
    for i, s in enumerate(spots):
        ...
        if f is None:
            soft("没填最佳观赏期 —— 有季节性的点（花期、丰水期、秋色）最好填上，"
                 "四季都长那样、什么时候去都行的点忽略这条")
